                ...
                <div class="table-responsive">
                    <table id="komarudin" class="table table-bordered table-hover">
                        ...
                        <tbody>
                            <?php
                            $no = 1;
                            $sql = mysqli_query($koneksi, "SELECT tbl_tags.*,tbl_gallery.* FROM tbl_gallery
                            INNER JOIN tbl_tags ON tbl_tags.id_tags=tbl_gallery.id_tags order by id_gallery desc");
                            while ($row = mysqli_fetch_array($sql)) {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <center>
                                            <a onclick="return confirm('Anda yakin akan menghapusnya ?')" title="hapus" href="admin.php?page=hapusgallery&id_gallery=<?php echo $row['id_gallery']; ?>" class="btn-sm btn-danger"><i class="fa fa-trash-o"></i> Hapus</a>
                                            <a title="edit" href="admin.php?page=editgallery&id_gallery=<?php echo $row['id_gallery']; ?>" class="btn-sm btn-info"><i class="fa fa-edit"></i> Edit</a>
                                        </center>
                                    </td>
                                </tr>
                                </tfoot>
                        ...
                        <div class="modal-content">
                            ...
                            <form action="" method="post" enctype="multipart/form-data">
                                <div class="modal-body">
                                    ...
                                    <div class="form-group">
                                        <label for="Induk" class="control-label">Kategori</label>
                                        <select name="id_tags" class="form-control">
                                            <?php
                                            $sql = mysqli_query($koneksi, "select * from tbl_tags");
                                            while ($data = mysqli_fetch_array($sql)) {
                                                echo "<option value='$data[id_tags]'> $data[judul_tags] </option>";
                                            }
                                            ?>
                                        </select>
                                    </div>
                            ...
                            if (isset($_POST['fsimpan'])) {
                                ...
                                $id_tags = htmlspecialchars($_POST['id_tags']);
                                ...
                                if (!$gambar) {
                                    ...
                                    $q = "INSERT INTO tbl_gallery(gambar, judul, id_tags, tanggal)
                                VALUES('$gambar', '$judul', '$id_tags', '$tanggal')";
